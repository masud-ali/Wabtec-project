<?php
include_once("../../helper/auth.php");
include_once("../../model/admin/AdminModel.php");
require_role('admin');
$model = new AdminModel();
$revenue = $model->getRevenueReport();
$volume = $model->getVolumeReport();
$performance = $model->getPerformanceReport();
?>
<!DOCTYPE html>
<html>
<head><title>Admin Reports</title><link rel="stylesheet" href="../../assets/style.css"></head>
<body>
<?php include_once("menu.php"); ?>
<div class="container">
<h2>Hospital-wide Revenue Report</h2>
<table><tr><th>Date</th><th>Doctor</th><th>Specialization</th><th>Revenue</th></tr><?php foreach($revenue as $r){ ?><tr><td><?php echo $r['paid_date']; ?></td><td><?php echo $r['doctor_name']; ?></td><td><?php echo $r['specialization']; ?></td><td><?php echo $r['total_revenue']; ?></td></tr><?php } ?></table>
<h2>Appointment Volume Report</h2>

<div class="stats-grid">
    <div class="card">
        <h3>Busiest Doctor</h3>
        <p><?php echo $volume['busiest_doctor']['name']; ?></p>
        <b>Total: <?php echo $volume['busiest_doctor']['total']; ?></b>
    </div>
    <div class="card">
        <h3>Most In-Demand Specialization</h3>
        <p><?php echo $volume['most_demanded_specialization']['name']; ?></p>
        <b>Total: <?php echo $volume['most_demanded_specialization']['total']; ?></b>
    </div>
    <div class="card">
        <h3>Peak Appointment Day</h3>
        <p><?php echo $volume['peak_day']['name']; ?></p>
        <b>Total: <?php echo $volume['peak_day']['total']; ?></b>
    </div>
    <div class="card">
        <h3>Peak Appointment Hour</h3>
        <p><?php echo $volume['peak_hour']['name']; ?></p>
        <b>Total: <?php echo $volume['peak_hour']['total']; ?></b>
    </div>
</div>

<h3>Busiest Doctors</h3>
<table>
    <tr><th>Doctor</th><th>Total Appointments</th></tr>
    <?php foreach($volume['doctor_count'] as $name => $total){ ?>
        <tr><td><?php echo $name; ?></td><td><?php echo $total; ?></td></tr>
    <?php } ?>
</table>

<h3>Most In-Demand Specializations</h3>
<table>
    <tr><th>Specialization</th><th>Total Appointments</th></tr>
    <?php foreach($volume['specialization_count'] as $name => $total){ ?>
        <tr><td><?php echo $name; ?></td><td><?php echo $total; ?></td></tr>
    <?php } ?>
</table>

<h3>Peak Appointment Days</h3>
<table>
    <tr><th>Day</th><th>Total Appointments</th></tr>
    <?php foreach($volume['day_count'] as $name => $total){ ?>
        <tr><td><?php echo $name; ?></td><td><?php echo $total; ?></td></tr>
    <?php } ?>
</table>

<h3>Peak Appointment Hours</h3>
<table>
    <tr><th>Hour</th><th>Total Appointments</th></tr>
    <?php foreach($volume['hour_count'] as $name => $total){ ?>
        <tr><td><?php echo $name; ?></td><td><?php echo $total; ?></td></tr>
    <?php } ?>
</table>

<h2>Doctor Performance Report</h2>
<table><tr><th>Doctor</th><th>Consultation Count</th><th>Average Rating</th><th>No-show</th></tr><?php foreach($performance as $p){ ?><tr><td><?php echo $p['doctor_name']; ?></td><td><?php echo $p['total_consultation']; ?></td><td><?php echo round($p['average_rating'],2); ?></td><td><?php echo $p['no_show_count']; ?></td></tr><?php } ?></table>
<button onclick="window.print()">Export / Print Report</button>
</div>
</body>
</html>
